--!nocheck
-- DEV / QA ONLY: full MistUI Test Lab. Do not ship this file to players.
--[[
    ============================================================
...
-- STANDALONE MIST HUB + COMPLETE TEST LAB
------------------------------------------------------------
print("[MistUI] full test standalone: starting")

pcall(function()
    Library:SetRemoteIconsEnabled(true)
end)
...
local Window = Library:CreateWindow({
    ...
    StatusText = "Game Name: v1.0.0",
})

assert(Window, "MistUI failed to create the main window")

local function makeCard(page, title, description, side)
    local row = page:CreateRow()
    return row:AddCard(title, description, { Side = side or "Left", Size = "Full" })
end

local function safeTest(card, label, callback)
    local ok, result = pcall(callback)
    if not ok then
        warn("[MistUI Test] " .. tostring(label) .. ": " .. tostring(result))
        pcall(function()
            card:AddStatus(label, "ERROR", "error")
            card:AddParagraph("Component error", tostring(result))
        end)
        return nil
    end
    return result
...
------------------------------------------------------------
-- NORMAL MIST HUB AREA
------------------------------------------------------------
local Scripts = Window:CreateCategory({
    Id = "mist.scripts",
    Name = "Scripts",
    Icon = "file",
    Description = "Mist Hub sections",
})
...
local normalPages = {
    {
        ...
        Build = function(page)
            local row = page:CreateRow()
            local left = row:AddCard("Movement", nil, { Side = "Left", Size = "Half" })
            left:Add("Toggle", { Id = "player.walkspeed", Text = "Walk Speed", Default = false })
            left:Add("Slider", { Id = "player.speed", Text = "Speed", Min = 16, Max = 250, Default = 16 })
            left:Add("Toggle", { Id = "player.jump", Text = "Jump Power", Default = false })
            local right = row:AddCard("Utility", nil, { Side = "Right", Size = "Half" })
            right:Add("Toggle", { Id = "player.noclip", Text = "No Clip", Default = false })
            right:Add("Keybind", { Id = "player.keybind", Text = "Player Keybind", Default = Enum.KeyCode.K, Mode = "Press" })
        end,
    },
    {
        ...
        Build = function(page)
            local row = page:CreateRow()
            local left = row:AddCard("Visual", nil, { Side = "Left", Size = "Half" })
            left:Add("Toggle", { Id = "esp.enabled", Text = "Enabled", Default = false })
            left:Add("Toggle", { Id = "esp.box", Text = "Box", Default = true })
            left:Add("ColorPicker", { Id = "esp.color", Text = "ESP Color", Default = Color3.fromRGB(248,248,249) })
            local right = row:AddCard("Distance", nil, { Side = "Right", Size = "Half" })
            right:Add("Slider", { Id = "esp.distance", Text = "Max Distance", Min = 100, Max = 5000, Default = 1500 })
        end,
    },
    {
        Id = "combat", Name = "Combat", Icon = "swords",
        Build = function(page)
            local card = makeCard(page, "Combat")
            card:Add("Toggle", { Id = "combat.enabled", Text = "Enabled", Default = false })
            card:Add("DropdownEx", { Id = "combat.mode", Text = "Mode", Options = { "Legit", "Rage", "Custom" }, Default = "Legit", Searchable = true })
            card:Add("Slider", { Id = "combat.fov", Text = "FOV", Min = 1, Max = 360, Default = 90 })
        end,
    },
    {
        Id = "teleport", Name = "Teleport", Icon = "map-pin",
        Build = function(page)
            local card = makeCard(page, "Teleport")
            card:Add("DropdownEx", { Id = "teleport.location", Text = "Location", Options = { "Spawn", "Area 1", "Area 2", "Area 3" }, Default = "Spawn", Searchable = true })
            card:Add("Button", { Text = "Teleport" })
        end,
    },
    {
        Id = "world", Name = "World", Icon = "globe",
        Build = function(page)
            local card = makeCard(page, "World")
            card:Add("Toggle", { Id = "world.option", Text = "World Option", Default = false })
            card:Add("Slider", { Id = "world.value", Text = "Value", Min = 0, Max = 100, Default = 50 })
        end,
    },
    {
        Id = "misc", Name = "Misc", Icon = "more-horizontal",
        Build = function(page)
            local card = makeCard(page, "Misc")
            card:Add("Input", { Id = "misc.input", Text = "Input", Placeholder = "Type something..." })
            card:Add("Button", { Text = "Test Notification", Callback = function()
                Library:Notify({ Title = "Mist Hub", Content = "Notification system is working.", Type = "Success", Duration = 3, Force = true })
            end })
        end,
    },
}

Scripts:AddGroup("Mist Hub", "settings", normalPages)

------------------------------------------------------------
-- COMPLETE TEST LAB
------------------------------------------------------------
local Tests = Window:CreateCategory({
    Id = "mist.tests",
    Name = "Tests",
    Icon = "settings",
    Description = "Complete MistUI component test laboratory",
})

Tests:AddScript("Basic", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Buttons & Feedback", "Basic visual controls and callbacks.", { Side = "Left", Size = "Half" })
    safeTest(left, "AddLabel", function() return left:AddLabel("Label component") end)
    safeTest(left, "AddParagraph", function() return left:AddParagraph("Paragraph", "Paragraph component with a longer description for wrapping.") end)
    safeTest(left, "AddSection", function() return left:AddSection("Section Header", "Section subtitle") end)
    safeTest(left, "AddDivider", function() return left:AddDivider("Divider") end)
    safeTest(left, "AddSeparator", function() return left:AddSeparator("Separator alias") end)
    safeTest(left, "AddBadge", function() return left:AddBadge("BADGE", "info") end)
    safeTest(left, "AddTag", function() return left:AddTag("TAG", "success") end)
    safeTest(left, "AddStatus", function() return left:AddStatus("Server", "ONLINE", "success") end)
    safeTest(left, "AddSpinner", function() return left:AddSpinner({ Text = "Spinner", Active = true }) end)
    safeTest(left, "AddProgressBar", function() return left:AddProgressBar("Progress", 45, 100) end)
    safeTest(left, "AddButton", function() return left:AddButton("Button", function()
        Library:Notify({ Title = "Button", Content = "AddButton callback fired.", Type = "Info", Duration = 2, Force = true })
    end) end)
    safeTest(left, "AddButtonEx", function() return left:AddButtonEx({ Text = "ButtonEx", Icon = "play", Variant = "primary", Callback = function()
        Library:Notify({ Title = "ButtonEx", Content = "Extended button works.", Type = "Success", Duration = 2, Force = true })
    end }) end)
    safeTest(left, "AddIconButton", function() return left:AddIconButton({ Icon = "settings", Tooltip = "Icon button", Callback = function() end }) end)
    safeTest(left, "AddButtonGroup", function() return left:AddButtonGroup({
        { Text = "One", Callback = function() end },
        { Text = "Two", Callback = function() end },
        { Text = "Three", Callback = function() end },
    }) end)
    safeTest(left, "AddConfirmButton", function() return left:AddConfirmButton({
        Text = "Confirm Button", ModalTitle = "Confirm test", ModalContent = "ConfirmButton modal test.", OnConfirm = function()
            Library:Notify({ Title = "Confirmed", Content = "ConfirmButton works.", Type = "Success", Duration = 2, Force = true })
        end,
    }) end)

    local right = row:AddCard("State Controls", "Common stateful components.", { Side = "Right", Size = "Half" })
    safeTest(right, "AddToggle", function() return right:AddToggle("Toggle", true, function() end) end)
    safeTest(right, "AddCheckbox", function() return right:AddCheckbox("Checkbox", false, function() end) end)
    safeTest(right, "AddSlider", function() return right:AddSlider("Slider", 0, 100, 35, function() end) end)
    safeTest(right, "AddRangeSlider", function() return right:AddRangeSlider("Range Slider", 0, 100, 20, 80, function() end) end)
    safeTest(right, "AddNumberBox", function() return right:AddNumberBox("Number Box", 0, 999, 42, 1, function() end) end)
    safeTest(right, "AddStepper", function() return right:AddStepper("Stepper", 1, 10, 3, 1, function() end) end)
    safeTest(right, "AddTextbox", function() return right:AddTextbox("Textbox", "Type here...", "Mist", function() end) end)
    safeTest(right, "AddInput", function() return right:AddInput({ Id = "test.input", Text = "Input", Placeholder = "Advanced input...", Default = "Hub" }) end)
end, "settings")

Tests:AddScript("Selectors", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Dropdowns", "Open these near page edges to test overlay/clipping behavior.", { Side = "Left", Size = "Half" })
    safeTest(left, "AddDropdown", function() return left:AddDropdown("Dropdown", { "One", "Two", "Three", "Four" }, "Two", function() end) end)
    safeTest(left, "AddDropdownEx", function() return left:AddDropdownEx({
        Id = "test.dropdownex", Text = "DropdownEx", Options = { "Alpha", "Bravo", "Charlie", "Delta", "Echo", "Foxtrot", "Golf" }, Default = "Bravo", Searchable = true, MaxVisibleDropdownItems = 5,
    }) end)
    safeTest(left, "AddSearchDropdown", function() return left:AddSearchDropdown("Search Dropdown", { "Apple", "Banana", "Cherry", "Dragon Fruit", "Grape", "Mango", "Orange" }, "Apple", function() end) end)
    safeTest(left, "AddMultiDropdown", function() return left:AddMultiDropdown({
        Id = "test.multidropdown", Text = "Multi Dropdown", Options = { "Alpha", "Bravo", "Charlie", "Delta", "Echo", "Foxtrot" }, Default = { "Alpha", "Charlie" }, Searchable = true, SummaryLimit = 2, MaxVisibleDropdownItems = 5,
    }) end)
    safeTest(left, "AddRadioGroup", function() return left:AddRadioGroup("Radio Group", { "One", "Two", "Three" }, "Two", function() end) end)
    safeTest(left, "AddSegmentedControl", function() return left:AddSegmentedControl("Segmented", { "Off", "Auto", "On" }, "Auto", function() end) end)

    local right = row:AddCard("Keybinds & Color", "Keybind list should show active entries white and inactive entries gray.", { Side = "Right", Size = "Half" })
    safeTest(right, "AddKeybind", function() return right:AddKeybind("Legacy Keybind", Enum.KeyCode.J, function() end) end)
    safeTest(right, "AddKeybindEx", function() return right:AddKeybindEx({ Id = "test.keybind.press", Text = "Press Keybind", Default = Enum.KeyCode.K, Mode = "Press" }) end)
    safeTest(right, "AddKeybindEx Toggle", function() return right:AddKeybindEx({ Id = "test.keybind.toggle", Text = "Toggle Keybind", Default = Enum.KeyCode.L, Mode = "Toggle" }) end)
    safeTest(right, "AddKeybindEx Hold", function() return right:AddKeybindEx({ Id = "test.keybind.hold", Text = "Hold Keybind", Default = Enum.KeyCode.H, Mode = "Hold" }) end)
    safeTest(right, "AddToggleKeybind", function() return right:AddToggleKeybind("Toggle + Keybind", true, Enum.KeyCode.T, function() end, "test.togglekeybind") end)
    safeTest(right, "AddColorPicker", function() return right:AddColorPicker({ Id = "test.color", Text = "Color Picker", Default = Color3.fromRGB(110, 90, 255), AllowTransparency = true, Transparency = 0.15 }) end)
end, "keyboard")

Tests:AddScript("Layout", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Containers", "Nested and collapsible layout components.", { Side = "Left", Size = "Half" })
    safeTest(left, "AddCollapsibleSection", function()
        local section = left:AddCollapsibleSection("Collapsible Section", true)
        section:AddLabel("Content inside the collapsible section.")
        section:AddButton("Inner Button", function() end)
        return section
    end)
    safeTest(left, "AddTabs", function()
        local tabs = left:AddTabs({ Tabs = { "General", "Visual", "Advanced" }, Default = "General" })
        for _, name in ipairs({ "General", "Visual", "Advanced" }) do
            local p = tabs:GetPage(name)
            if p then
                ...
                label.Text = name .. " tab content"
                label.Parent = p
            end
        end
        return tabs
    end)
    safeTest(left, "AddAccordion", function() return left:AddAccordion({
        { Title = "First", Content = "First accordion content" },
        { Title = "Second", Content = "Second accordion content" },
        { Title = "Third", Content = "Third accordion content" },
    }) end)
    safeTest(left, "AddBreadcrumb", function() return left:AddBreadcrumb({ "Home", "Tests", "Layout" }, function() end) end)

    local right = row:AddCard("Dependency & Data", "Lists, tables, trees and dependency behavior.", { Side = "Right", Size = "Half" })
    safeTest(right, "AddDependencyBox", function()
        local source = right:AddToggle("Show Dependency Box", true, function() end)
        local box = right:AddDependencyBox({ Title = "Dependency Box", Bordered = true })
        box:Add("Toggle", { Id = "test.dep.toggle", Text = "Dependent Toggle", Default = true })
        box:Add("Slider", { Id = "test.dep.slider", Text = "Dependent Slider", Min = 0, Max = 100, Default = 50 })
        box:SetupDependencies(source)
        return box
    end)
    safeTest(right, "AddList", function() return right:AddList({ Text = "List", Items = { "Item 1", "Item 2", "Item 3", "Item 4", "Item 5", "Item 6", "Item 7" }, MaxVisible = 4 }) end)
    safeTest(right, "AddVirtualList", function()
        local items = {}
        for i = 1, 100 do items[i] = "Virtual Item " .. tostring(i) end
        return right:AddVirtualList({ Text = "Virtual List", Items = items, Height = 150, RowHeight = 28 })
    end)
    safeTest(right, "AddTable", function() return right:AddTable({ Columns = { "Component", "State" }, Rows = { { "Dropdown", "OK" }, { "ColorPicker", "OK" }, { "Config", "TEST" } } }) end)
    safeTest(right, "AddTreeView", function() return right:AddTreeView({ Nodes = {
        { Text = "Root", Children = { { Text = "Child A" }, { Text = "Child B", Children = { { Text = "Nested" } } } } },
    } }) end)
end, "file")

Tests:AddScript("Media", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Media", "Image, avatar, video and viewport components.", { Side = "Left", Size = "Half" })
    local imageApi = safeTest(left, "AddImage", function() return left:AddImage({ Image = "", Height = 120 }) end)
    if imageApi and LocalPlayer then
        task.spawn(function()
            local ok, url = pcall(function()
                return Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
            end)
            if ok and imageApi.SetImage then imageApi:SetImage(url) end
        end)
    end
    safeTest(left, "AddAvatar", function() return left:AddAvatar({ Text = LocalPlayer and LocalPlayer.Name or "Player", UserId = LocalPlayer and LocalPlayer.UserId or 1, Size = 52 }) end)
    safeTest(left, "AddVideo", function() return left:AddVideo({ Text = "Video", Video = "", Height = 100, Playing = false, Looped = true }) end)

    local right = row:AddCard("3D & Custom UI", "Interactive Viewport and UI passthrough.", { Side = "Right", Size = "Half" })
    safeTest(right, "AddViewport", function()
        local part = Instance.new("Part")
        part.Name = "MistUITestPart"
        part.Size = Vector3.new(3, 3, 3)
        part.Anchored = true
        part.Material = Enum.Material.Neon
        part.Color = Color3.fromRGB(110, 90, 255)
        return right:AddViewport({ Text = "Viewport", Object = part, Clone = true, Height = 180, Interactive = true })
    end)
    safeTest(right, "AddUIPassthrough", function()
        local frame = Instance.new("Frame")
        frame.BackgroundColor3 = Color3.fromRGB(110, 90, 255)
        ...
        label.Text = "UIPassthrough Test"
        label.TextColor3 = Color3.new(1, 1, 1)
        label.TextSize = 15
        label.Parent = frame
        return right:AddUIPassthrough({ Text = "Custom Frame", Instance = frame, Clone = true, Height = 56 })
    end)
    safeTest(right, "AddCodeBox", function() return right:AddCodeBox({ Text = [[print("MistUI Test Lab")
return true]], Height = 92 }) end)
    safeTest(right, "AddFormattedText", function() return right:AddFormattedText("**Bold**  __Underline__  `Code`") end)
end, "eye")

Tests:AddScript("Activity", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Live Activity HUD", "Long-running task HUDs that update without recreating the interface.", { Side = "Left", Size = "Half" })

    left:AddButton("Show Auto Boss Activity", function()
        local activity = Window:CreateActivity({
            Title = "Auto Boss",
            Icon = "swords",
            CounterLabel = "Bosses defeated",
            Closable = true,
            Minimizable = true,
            AutoTimer = true,
        })
        activity:SetCounter(0, 10, "Bosses defeated")
        activity:SetTarget("Ancient Guardian", "Target")
        activity:SetStatus("Moving to target...", "info")
        activity:SetProgress(0, "0%")

        task.spawn(function()
            local bosses = { "Ancient Guardian", "Crimson Warden", "Abyss Knight", "Storm Tyrant" }
            local defeated = 0
            while activity.Instance and activity.Instance.Parent and defeated < 10 do
                local boss = bosses[(defeated % #bosses) + 1]
                activity:SetTarget(boss, "Target")
                activity:SetStatus("Fighting boss...", "warning")
                for step = 0, 100, 4 do
                    if not activity.Instance or not activity.Instance.Parent then return end
                    activity:SetProgress(step / 100)
                    task.wait(0.12)
                end
                defeated += 1
                activity:SetCounter(defeated, 10, "Bosses defeated")
                activity:SetStatus(defeated >= 10 and "Completed" or "Moving to next boss...", defeated >= 10 and "success" or "info")
                task.wait(0.45)
            end
        end)
    end)

    left:AddButton("Show Detector Activity", function()
        local activity = Window:CreateActivity({
        ...
        local chakra = activity:AddSection({
            Title = "Chakra Sense",
            Icon = "eye",
            Counter = 1,
        })
        local detected = chakra:AddStatus({
            Title = "Detected",
            Description = "1 player",
            Status = "warning",
        })
        local playerRow = chakra:AddRow({
            Title = "Viraviria",
            Value = "142",
            Status = "warning",
        })

        local staff = activity:AddSection({
            Title = "Staff Detector",
            Icon = "shield",
        })
        local staffStatus = staff:AddStatus({
            Title = "Clear",
        ...
        task.spawn(function()
            local count = 1
            while activity.Instance and activity.Instance.Parent do
                task.wait(2)
                if not activity.Instance or not activity.Instance.Parent then return end
                count = count == 3 and 1 or count + 1
                chakra:SetCounter(count)
                detected:SetDescription(tostring(count) .. (count == 1 and " player" or " players"))
                playerRow:SetValue(tostring(math.random(90, 220)))
                if count == 3 then
                    staffStatus:SetTitle("Checking...")
                    staffStatus:SetDescription("Scanning server")
                    staffStatus:SetStatus("info")
                else
                    staffStatus:SetTitle("Clear")
                    staffStatus:SetDescription("No staff in server")
                    staffStatus:SetStatus("success")
                end
            end
        end)
    end)

    left:AddButton("Close All Activities", function()
        Window:CloseActivities(false)
    end)

    local right = row:AddCard("Activity API", "The same component can build Auto Boss, quest, detector or farm dashboards.", { Side = "Right", Size = "Half" })
    right:AddParagraph("Live updates", "SetCounter, SetTarget, SetStatus, SetProgress and timers update the existing HUD in real time.")
    right:AddParagraph("Sections", "AddSection can contain AddStatus and AddRow items with independent values and state colors.")
    right:AddParagraph("Player friendly", "Activities are draggable, minimizable, closable and never block the main Mist interface.")
end, "info")

Tests:AddScript("Overlays", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Overlays", "Modal, drawer, popup, context menu and command palette.", { Side = "Left", Size = "Half" })

    safeTest(left, "AddActionMenu", function() return left:AddActionMenu({ Text = "Action Menu", Items = {
        { Text = "Copy", Callback = function() end },
        { Text = "Duplicate", Callback = function() end },
        { Text = "Delete", Callback = function() end },
    } }) end)

    left:AddButton("Open Modal", function()
        local modal = Window:CreateModal({ Size = UDim2.new(0, 420, 0, 240), CloseOnBackdrop = true })
        modal:SetTitle("Modal Test")
        modal:AddText("Click outside or close the modal to test overlay cleanup.")
        modal:AddButton("Close", function() modal:Close() end)
    end)
    left:AddButton("Open Drawer", function()
        local drawer = Window:CreateDrawer({ Side = "right", Width = 320 })
        drawer:SetTitle("Drawer Test")
        drawer:AddText("Drawer component is working.")
        drawer:AddButton("Close", function() drawer:Close() end)
    end)
    left:AddButton("Open Popup", function()
        local popup = Window:CreatePopup({ Size = UDim2.new(0, 340, 0, 180), Position = UDim2.new(0.5, -170, 0.5, -90) })
        popup:SetTitle("Popup Test")
        popup:AddText("Popup without a mandatory backdrop.")
        popup:AddButton("Close", function() popup:Close() end)
    end)
    left:AddButton("Open Confirm", function()
        Window:Confirm({ Title = "Confirm Test", Content = "This tests the confirmation overlay.", ConfirmText = "Confirm", CancelText = "Cancel", OnConfirm = function()
            Library:Notify({ Title = "Confirm", Content = "Confirmed.", Type = "Success", Duration = 2, Force = true })
        end })
    end)

    local contextApi
    contextApi = left:AddButtonEx({ Text = "Open Context Menu", Icon = "more-horizontal", Callback = function()
        Window:CreateContextMenu({
            { Text = "Action One", Callback = function() end },
            { Text = "Action Two", Callback = function() end },
            { Text = "Action Three", Callback = function() end },
        }, contextApi and contextApi.Button or nil)
    end })

    left:AddButton("Open Command Palette", function() Window:OpenCommandPalette({ Title = "Mist Test Commands" }) end)
    left:AddButton("Open Debug Panel", function() Window:OpenDebugPanel({ Title = "Mist Developer Tools" }) end)

    local right = row:AddCard("Notifications", "All notification states and management.", { Side = "Right", Size = "Half" })
    for _, kind in ipairs({ "Info", "Success", "Warning", "Error" }) do
        right:AddButton(kind .. " Notification", function()
            Library:Notify({ Title = kind, Content = kind .. " notification test.", Type = kind, Duration = 4, Force = true, Closable = true })
        end)
    end
    right:AddButton("Notification With Action", function()
        Library:Notify({ Title = "Update available", Content = "Action button test.", Type = "Info", Duration = 6, Force = true, ActionText = "Apply", OnAction = function()
            Library:Notify({ Title = "Action", Content = "Notification action fired.", Type = "Success", Duration = 2, Force = true })
        end })
    end)
    right:AddButton("Dismiss Latest", function() Library:DismissLatestNotification(false) end)
    right:AddButton("Clear Notifications", function() Library:ClearNotifications() end)
end, "star")

Tests:AddScript("System", function(page)
    local row = page:CreateRow()
    local left = row:AddCard("Interface", "Window and keybind-list behavior. Manual resizing is intentionally disabled.", { Side = "Left", Size = "Half" })
    left:AddStatus("Manual Resize", Window:IsResizable() and "ENABLED" or "DISABLED", Window:IsResizable() and "warning" or "success")
    left:AddButton("Toggle Keybind List", function() Window:SetKeybindMenuOpen(not Window:GetKeybindMenuOpen()) end)
    left:AddButton("Toggle Interface", function() Window:Toggle() end)
    left:AddButton("Center Window", function() Window:Center() end)
    left:AddButton("Scale 90%", function() Window:SetScale(0.9) end)
    left:AddButton("Scale 100%", function() Window:SetScale(1.0) end)
    left:AddButton("Reduced Motion ON", function() Library:SetReducedMotion(true) end)
    left:AddButton("Reduced Motion OFF", function() Library:SetReducedMotion(false) end)

    local right = row:AddCard("Diagnostics & Config", "Development-only checks for configs and library health.", { Side = "Right", Size = "Half" })
    right:AddButton("Run Doctor", function()
        local report = Window:Doctor()
        Library:Notify({ Title = "Mist Doctor", Content = "Health score: " .. tostring(report.Score) .. "/100", Type = report.Score >= 90 and "Success" or "Warning", Duration = 4, Force = true })
    end)
    right:AddButton("Run Quality Suite", function()
        local ok, result = pcall(function() return Window:RunQualitySuite() end)
        Library:Notify({ Title = "Quality Suite", Content = ok and "Suite finished. Check console/debug panel." or tostring(result), Type = ok and "Success" or "Error", Duration = 5, Force = true })
    end)
    right:AddButton("Show Config Count", function()
        local ok, configs = pcall(function() return Window:GetConfigs() end)
        local count = ok and type(configs) == "table" and #configs or 0
        Library:Notify({ Title = "Configuration", Content = ok and ("Persistent configs: " .. tostring(count)) or tostring(configs), Type = ok and "Info" or "Error", Duration = 4, Force = true })
    end)
    right:AddButton("Reset Test Flags", function()
        for flag in pairs(Library:GetFlags()) do
            if tostring(flag):find("test", 1, true) then Library:SetFlag(flag, nil, true) end
        end
        Library:Notify({ Title = "Test Lab", Content = "Test flags reset.", Type = "Info", Duration = 2, Force = true })
    end)
end, "info")

Window:RegisterCommand({ Id = "goto_tests_basic", Name = "Open Test Lab: Basic", Description = "Opens the Basic component test page.", Keywords = { "test", "basic", "components" }, Callback = function()
    if Tests._items[1] and Tests._items[1]._open then Tests._items[1]._open() end
end })
Window:RegisterCommand({ Id = "test_notification", Name = "Test Notification", Description = "Shows a sample notification.", Keywords = { "notification", "toast" }, Callback = function()
    Library:Notify({ Title = "Mist Test", Content = "Command palette action fired.", Type = "Info", Duration = 3, Force = true })
end })

-- Open the complete Test Lab immediately so testing can start without navigation.
if Tests._items[1] and Tests._items[1]._open then
    Tests._items[1]._open()
end

print("[MistUI] manual resize:", Window:IsResizable())
print("[MistUI] full test lab ready. Pages: Basic, Selectors, Layout, Media, Activity, Overlays, System")
